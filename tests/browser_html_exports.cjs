...
(async () => {
  ...
  try {
    ...
    assert(fixture.report.includes('Race tyre sets'));
    assert(fixture.report.includes('No finite race set ledger recorded.'));
    assert(fixture.report.includes('Completed race suspension'));
    assert(fixture.report.includes('Not recorded'));
    assert(fixture.comparison.includes('Input race set pools'));
    assert(fixture.comparison.includes('Completed race suspension'));
    assert(fixture.comparison.includes('Race-wide collection + restart pause'));
    ...
    const pairedTime = page.getByRole('region', {name: 'A elapsed time changes', exact: true});
    const timeStats = stats.finished_race_time;
    const timeText = await pairedTime.innerText();
    assert(timeText.includes(`${timeStats.paired_races} same-distance finish pairs`));
    assert(timeText.includes(`${signed(timeStats.mean_seconds_difference)} s`));
    assert(timeText.includes(`SE ${timeStats.seconds_difference_standard_error.toFixed(3)} s`));
