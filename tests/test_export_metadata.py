...
def test_statistics_export_includes_run_metadata(tmp_path) -> None:
    ...
    assert "mechanical_tuning_suggestions" in data
    assert "reliability_adjustment_recommendations" in data

    ver = data["driver_statistics"]["VER"]
    assert "position_percentiles" in ver
    assert "top_5_finish_probability" in ver
    assert "top_10_finish_probability" in ver
